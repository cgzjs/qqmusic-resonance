...
export function SongExchange({ listener, selectedSongId, status, receivedTrack, onSelectSong, onSend, onBack, onBrowse, onSave, onListenLater, isSaving = false }: SongExchangeProps) {
  ...
  if (status === "received" && receivedTrack) return (
    <section className="screen-view exchange-result">
      <ViewHeader eyebrow="交换完成" title="收到一首新音乐" onBack={onBack} />
      ...
      <article className="received-song"><AlbumTile coverUrl={receivedTrack.coverUrl} accent={receivedTrack.accent} size="lg" /><p>FOR YOU</p><h3>{receivedTrack.track}</h3><span>{receivedTrack.artist}</span><blockquote>这是 TA 回应你的音乐</blockquote></article>
  ...
  if (isSending) return <section className="screen-view exchange-waiting">
    <ViewHeader eyebrow="交换一首" title="歌已送出，等 TA 回一首" onBack={onBack} />
    <ExchangeSignal sending />
    <p className="exchange-waiting-song">你送出了《{selectedSong?.track}》</p>
    <p className="exchange-intro" role="status">对方正在挑歌，回应到了会提醒你。</p>
    <Button className="primary-action" onClick={onBrowse}>先去逛逛</Button>
    <p className="demo-notice">不用守在这里，回歌到了会提醒你。</p>
  </section>;
  return (
    <section className="screen-view">
      <ViewHeader eyebrow="交换一首" title="从你的世界里挑一首" onBack={onBack} />
      <ExchangeSignal />
      <p className="exchange-intro">从当前歌单挑一首，体验一次匿名音乐交换。</p>
      ...
      <Button className="primary-action exchange-send" disabled={!selectedSong} onClick={onSend}><Send aria-hidden="true" />匿名送出这首歌</Button>
    </section>
  );
}
