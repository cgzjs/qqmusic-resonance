...
type Props = { active: boolean; track?: AudioTrack; listeners: NearbyListener[]; selectedId: string; onSelect: (listener: NearbyListener) => void };

export function OrbitRadar({ active, track, listeners, selectedId, onSelect }: Props) {
  ...
  return <section className="orbit-map" aria-label="附近音乐雷达" data-active={active}>
    <svg className="orbit-lines" viewBox="0 0 400 400" aria-hidden="true">
      ...
      {selected && <><path className="orbit-connection" d={route} />{active && <circle className="orbit-signal" r="3"><animateMotion dur="3.8s" repeatCount="indefinite" path={route} /></circle>}</>}
    </svg>
    <div className="orbit-self" aria-label={`你正在听${track ? ` ${track.track}` : "的音乐"}`}>
      <span className="orbit-vinyl" aria-hidden="true"><i /></span>
      {track ? <AlbumTile coverUrl={track.coverUrl} accent={track.accent} size="md" /> : <Headphones size={28} aria-hidden="true" />}
      <span className="orbit-self-label">我在听</span>
    </div>
